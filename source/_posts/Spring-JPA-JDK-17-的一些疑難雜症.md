...
遇到時當下我是有點困惑的，因為之前做時從來沒看過，在這邊做個紀錄：
錯誤的地方如下：
```
...
```
這很明顯的就是少了設定資料庫的poperties，所以應該要在application.poperties添加以下config
```
...
```
從上至下分別是：連線資料庫的url、連線資料庫的帳號、密碼、以及資料庫驅動程式，根據你使用不同的資料庫要添加的稍有不同，如果是使用SQL Server的話應該這樣就夠了。

### 問題三
如果在連線時出現以下問題，代表資料庫你是沒有設定SSL加密的，方法有兩種，要不你就去調整資料庫SSL加密，要不你就在application.poperties取消加密。如果後者可以參考以下程式。
```
